        ...
        const PREVIEW_STORAGE_KEY = 'amazoniaPreviewSettings';
        const PREVIEW_SIZES = {
            mobile: { width: 375, height: 667, label: 'Móvil', breakpoint: '≤640px' },
            tablet: { width: 834, height: 1112, label: 'Tablet', breakpoint: '641px - 1024px' },
            desktop: { width: 1280, height: 720, label: 'Escritorio', breakpoint: '≥1025px' }
        };
        ...
        let previewStorage = null;
        let previewFrameElement = null;
        let previewDeviceElement = null;
        let previewBreakpointBadge = null;
        let previewOverflowAlert = null;
        ...
        function loadPreviewPreferences() {
            ...
            try {
                const storedValue = previewStorage.getItem(PREVIEW_STORAGE_KEY);
                if (!storedValue) {
                    return previewPreferences;
                }
                ...
                return {
                    size: PREVIEW_SIZES[parsed.size] ? parsed.size : previewPreferences.size,
                    orientation: parsed.orientation === 'horizontal' ? 'horizontal' : previewPreferences.orientation,
                    zoom: clampPreviewZoom(Number(parsed.zoom))
                };
            } catch (error) {
                console.warn('No se pudieron cargar las preferencias de vista previa', error);
                return previewPreferences;
            }
        }
        ...
        function persistPreviewPreferences() {
            ...
            try {
                previewStorage.setItem(PREVIEW_STORAGE_KEY, JSON.stringify(previewPreferences));
            } catch (error) {
                console.warn('No se pudieron guardar las preferencias de vista previa', error);
            }
        }
        ...
        function clampPreviewZoom(value) {
            ...
            if (!Number.isFinite(value)) {
                return previewPreferences.zoom || 1;
            }

            return Math.min(max, Math.max(min, value));
        }

        function getPreviewTargetDimensions(preferences = previewPreferences) {
            const preset = PREVIEW_SIZES[preferences.size] || PREVIEW_SIZES.desktop;
            const orientation = preferences.orientation === 'horizontal' ? 'horizontal' : 'vertical';
        ...
        function updateZoomDisplay() {
            ...
            if (zoomValue) {
                zoomValue.textContent = `${Math.round(previewPreferences.zoom * 100)}%`;
            }
        }

        function updateBreakpointBadge() {
            if (!previewBreakpointBadge || !previewBreakpointBadge.isConnected) {
                previewBreakpointBadge = document.getElementById('previewBreakpointBadge');
            }

            const preset = PREVIEW_SIZES[previewPreferences.size] || PREVIEW_SIZES.desktop;
            if (previewBreakpointBadge) {
                previewBreakpointBadge.textContent = `${preset.label} (${preset.breakpoint})`;
            }
        }

        function applyPreviewPreferences(preferences = previewPreferences) {
            if (!previewFrameElement || !previewFrameElement.isConnected) {
                previewFrameElement = document.getElementById('catalogPreview');
            }

            if (!previewDeviceElement || !previewDeviceElement.isConnected) {
                previewDeviceElement = document.getElementById('previewDevice');
            }

            if (!previewFrameElement || !previewDeviceElement) {
                return;
            }
            ...
            const validatedPrefs = {
                ...previewPreferences,
                ...preferences,
                size: PREVIEW_SIZES[preferences.size] ? preferences.size : previewPreferences.size,
                orientation: preferences.orientation === 'horizontal' ? 'horizontal' : 'vertical',
                zoom: clampPreviewZoom(preferences.zoom)
            };

            previewPreferences = validatedPrefs;

            const { width, height } = getPreviewTargetDimensions(previewPreferences);
            previewDeviceElement.style.width = `${width}px`;
            previewDeviceElement.style.height = `${height}px`;
            previewDeviceElement.style.setProperty('--preview-scale', previewPreferences.zoom);
            previewDeviceElement.dataset.orientation = previewPreferences.orientation;

            const sizeButtons = document.querySelectorAll('[data-preview-size]');
            sizeButtons.forEach(button => {
                const size = button.getAttribute('data-preview-size');
                const isActive = size === previewPreferences.size;
                button.classList.toggle('preview-chip--active', isActive);
                button.setAttribute('aria-pressed', isActive ? 'true' : 'false');
            });

            const orientationButton = document.getElementById('previewOrientationToggle');
            if (orientationButton) {
                const isHorizontal = previewPreferences.orientation === 'horizontal';
                orientationButton.textContent = isHorizontal ? '↕️ Vertical' : '↔️ Horizontal';
                orientationButton.setAttribute('aria-pressed', isHorizontal ? 'true' : 'false');
            }

            updateZoomDisplay();
            updateBreakpointBadge();
            evaluatePreviewOverflow();
        ...
        function setPreviewSize(size) {
            ...
            applyPreviewPreferences({ ...previewPreferences, size });
        }

        function togglePreviewOrientation() {
            const nextOrientation = previewPreferences.orientation === 'horizontal' ? 'vertical' : 'horizontal';
            applyPreviewPreferences({ ...previewPreferences, orientation: nextOrientation });
        }

        function changePreviewZoom(delta) {
            const nextZoom = clampPreviewZoom(previewPreferences.zoom + delta);
            applyPreviewPreferences({ ...previewPreferences, zoom: nextZoom });
        }

        function resetPreviewZoom() {
            applyPreviewPreferences({ ...previewPreferences, zoom: 1 });
        }

        function evaluatePreviewOverflow() {
            if (!previewOverflowAlert || !previewOverflowAlert.isConnected) {
                previewOverflowAlert = document.getElementById('previewOverflowAlert');
            }

            if (!previewOverflowAlert) {
                return;
            }

            if (previewPreferences.size !== 'mobile' || !previewFrameElement || !previewFrameElement.contentDocument) {
                previewOverflowAlert.hidden = true;
                return;
            }

            try {
                const doc = previewFrameElement.contentDocument;
                const viewportWidth = doc.documentElement.clientWidth || doc.body.clientWidth;
                const offenders = Array.from(doc.body.querySelectorAll('*')).filter(element => {
                ...
                const hasOverflow = doc.documentElement.scrollWidth - viewportWidth > 2 || offenders.length > 0;

                if (hasOverflow) {
                    if (offenders.length > 0) {
                        const offenderLabels = offenders
                            .map(el => el.getAttribute('data-product-id') || el.className || el.tagName)
                            .join(', ');

                        previewOverflowAlert.innerHTML = `<strong>⚠️ Atención:</strong> Detectamos elementos que se desbordan en el modo móvil${offenderLabels ? ` (pista: ${offenderLabels})` : ''}. Revisa los márgenes o textos demasiado largos.`;
                    }

                    previewOverflowAlert.hidden = false;
                } else {
                    previewOverflowAlert.hidden = true;
                }
            } catch (error) {
        ...
        function initializePreviewControls() {
            previewStorage = getPersistentStorage();
            previewFrameElement = document.getElementById('catalogPreview');
            previewDeviceElement = document.getElementById('previewDevice');
            previewBreakpointBadge = document.getElementById('previewBreakpointBadge');
            previewOverflowAlert = document.getElementById('previewOverflowAlert');

            previewPreferences = { ...previewPreferences, ...loadPreviewPreferences() };
            ...
            const orientationButton = document.getElementById('previewOrientationToggle');
            if (orientationButton) {
                orientationButton.addEventListener('click', togglePreviewOrientation);
            }

            const zoomButtons = document.querySelectorAll('[data-zoom-step]');
            zoomButtons.forEach(button => {
                button.addEventListener('click', () => {
                    const step = Number.parseFloat(button.getAttribute('data-zoom-step'));
                    if (Number.isFinite(step)) {
                        changePreviewZoom(step);
                    }
                });
            });

            const resetButton = document.getElementById('previewZoomReset');
            if (resetButton) {
                resetButton.addEventListener('click', resetPreviewZoom);
            }

            if (previewFrameElement) {
                previewFrameElement.addEventListener('load', () => {
                    applyPreviewPreferences(previewPreferences);
                    evaluatePreviewOverflow();
                });
            }
